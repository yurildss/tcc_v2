...
# augmentation_ecg.py
# =============================================================================
# Gera amostras aumentadas para balancear o dataset de batimentos ECG.
#
# COMPORTAMENTO:
#   - L e N não recebem augmentation (já atingem a meta com originais)
#   - N é subsampleado para 10000
#   - As demais classes recebem augmentation com COMBINAÇÃO de técnicas
#   - Originais e aumentados ficam na mesma pasta por classe
#
# METAS:
#   A =  6000 (orig: 2544 → aug: 3456)
#   B =  8000 (orig: 7024 → aug:  976)
#   F =  4000 (orig:  803 → aug: 3197)
#   L =  8075 (orig: 8072 → sem aug, copia tudo)
#   N = 10000 (orig:75016 → subsample 10000, sem aug)
#   R =  8000 (orig: 7256 → aug:  744)
#   V =  8000 (orig: 7130 → aug:  870)
#   f =  4000 (orig:  982 → aug: 3018)
#
# TÉCNICAS (combinadas aleatoriamente por amostra):
#   TimeShift      — desloca sample# horizontalmente (±20 amostras)
#   Jitter         — ruído gaussiano leve
#   AmplitudeScale — escala de amplitude ±15%
#   BaselineWander — deriva senoidal de baixa frequência (respiração)
#   Smooth         — suavização leve por média móvel
#
# COMBINAÇÃO:
#   Cada amostra aug sorteia aleatoriamente entre 1 e 3 técnicas
#   que são aplicadas em sequência. A probabilidade de cada técnica
#   estar na combinação é independente.
#
# SAÍDA:
#   dataset_aumentado/<classe>/
#     <classe>_beat_<N>_<record>.csv    ← originais
#     <classe>_aug_<N>.csv              ← aumentados
# =============================================================================
...
def aplicar_combinacao(signal, samples):
    """
    Sorteia aleatoriamente entre 1 e 3 técnicas e aplica em sequência.
    Garante que pelo menos uma técnica que modifica o sinal está presente.
    Retorna (signal_aug, samples_aug, lista_de_tecnicas_usadas).
    """
    todas = list(TECNICAS_FN.keys())

    # Número de técnicas nesta combinação: 1, 2 ou 3
...
for cls in sorted(METAS.keys()):
    ...
    if n_orig >= meta:
        # Subsamplea (caso N ou L com excesso)
